...
class RF233Radio(BPHandler):
    ...
    def read_len(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
    ...
    @bp_handler(['rf233_on'])
    def on(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
        # int rf233_on(void);
        log.debug("rf233_on")
    ...
    def get_channel(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
        # int rf_get_channel(void);
        log.debug("rf_get_channel")
        return True, 0

    @bp_handler(['rf_set_channel'])  # used
    def set_channel(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
        # int rf_set_channel(uint8_t ch);
        log.debug("rf_set_channel")
        return True, 0

    @bp_handler(['SetIEEEAddr'])
    def SetIEEEAddr(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
    ...
    def trx_reg_read(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
        reg = qemu.regs.r0
    ...
    def trx_spi_init(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
    ...
    def trx_reg_write(self, qemu: HALQemuTarget, bp_addr: int) -> HandlerReturn:
