...
class Timers(BPHandler, AvatarPeripheral):
    ...
    def __init__(self, model=TimerModel):
        ...
        self.name2isr_lut = {'Timer0': 0x20, 'Timer1': 0x21,
                             'Timer3': 0x22, 'Timer4': 0x23, 'Timer5': 0x24}
        self.irq_rates = {}
        self.name = 'Timer'
        self._init_mmio()
    ...
    def hw_read(self, offset, size, pc, **kwargs):
        ...
        log.info("Read from addr: 0x%08x, offset:0x%04x size: %i value: 0x%0x, pc: %s" % (
    ...
    def enable(self, qemu, bp_addr):
        ...
        for irq_name, irq_rate in list(self.irq_rates.items()):
            self.model.start_timer(
                irq_name, self.name2isr_lut[irq_name], irq_rate)
