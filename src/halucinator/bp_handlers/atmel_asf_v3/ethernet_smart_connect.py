...
class EthernetSmartConnect(BPHandler):
    ...
    def eth_process(self, qemu: "HalBackend", bp_addr: int) -> HandlerReturn:
        ...
        if num_frames > 0:
            ...
            buf_ptr = qemu.read_memory(0x200000e8, 4, 1)  # ip_packet_bufer
            frame = self.model.get_rx_frame_only(self.get_id(qemu))
            frame_len = len(frame)
            qemu.write_memory(buf_ptr, 1, frame, frame_len, raw=True)
    ...
    def read(self, qemu: "HalBackend", bp_addr: int) -> HandlerReturn:
        ...
        if num_frames > 0:
            ...
            frame, rx_time = self.model.get_rx_frame_and_time(self.get_id(qemu))
            frame_length = len(frame)
            if max_len >= frame_length:
                log.info("Frame Read w len %i", frame_length)
                qemu.write_memory(data_ptr, 1, frame, len(frame), raw=True)
