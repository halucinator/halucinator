...
controller firmware. **The firmware binaries are not shipped in this
repository** — `Drone.elf` and `Drone.bin` are compiled at test time
by `build_drone.sh` from a pinned upstream commit, and `drone_addrs.yaml`
is regenerated by halucinator's own `hal_make_addr` tool against the
freshly-built ELF. This file records every upstream attribution I could
trace, since the upstream license situation is itself messy (see below).
The trace came from reading strings out of a reference ELF that was
briefly checked in during development, then walking the authors'
public repositories.

---

## Build flow in this repository

`build_drone.sh` clones
...
(pinned, 2020-12-11) and compiles `Drone/` with:

| Setting | Value                                                      |
|---------|------------------------------------------------------------|
| MCU     | STM32F103C8 (Cortex-M3, Thumb-2, no FPU)                    |
| CC      | `arm-none-eabi-gcc` (whatever the CI toolchain provides)   |
| Flags   | `-mcpu=cortex-m3 -mthumb -mfloat-abi=soft -Os -g3 -ffunction-sections -fdata-sections` |
| Defines | `USE_HAL_DRIVER`, `STM32F103xB`, `__weak`, `__packed`       |
| Linker  | `STM32F103C8_FLASH.ld` (from upstream) + `-Wl,--gc-sections` + `--specs=nano.specs` |

These flags are a plain translation of the Atollic TrueSTUDIO Debug
configuration in `Drone/.cproject`. The resulting ELF's reset vector
is extracted from `.isr_vector` and patched into `drone_memory.yaml`
so the halucinator config always matches the build. The symbol
address file `drone_addrs.yaml` is regenerated via
`hal_make_addr -b Drone.elf -o drone_addrs.yaml`; the intercept yaml
only names `function:` entries, so address resolution happens at
halucinator load time against the freshly-regenerated map.
...
**Option 3 from the original checklist** — removed the prebuilt
binaries and the symbol file from the repository (and from the
branch's git history, via `git filter-repo`), and now regenerate them
from pinned upstream source at test time. This sidesteps the
redistribution question: halucinator itself no longer ships the
unlicensed artifact. Anyone running the test fetches upstream under
GitHub's default terms of service (viewing / building for personal
use is implicitly permitted; redistributing the compiled artifact
remains the user's decision to make against upstream's license state).

If upstream ever adds a LICENSE file, this script can simply be
updated to copy it here.

### Alternatives still available if the upstream license situation
is later resolved adversely:
...
1. **Contact the authors** — the [e-Yantra project](https://www.e-yantra.org/)
   ...
   halucinator's research-tool conventions).
