...
struct ConfigurableMachineState {
    /* Private */
...
static void init_memory_area(QDict *mapping, const char *config_filename)
{
    uint64_t size;
    uint64_t data_size;
    char * data = NULL;
    ...
    if (qdict_haskey(mapping, "file"))
    {
        int file;
        const char * filename;
        int dirname_len = get_dirname_len(config_filename);
        ssize_t err;
        ...
        if (!is_absolute_path(filename))
        {
            ...
            strncpy(relative_filename, config_filename, dirname_len);
...
static void connect_irqs(QDict * dev_dict, QDict * periphs){
...
{
    THISCPU *cpuu;
    ConfigurableMachineState * cms = CONFIGURABLE_MACHINE(ms);
    ...
    if (qdict_haskey(conf, "memory_mapping"))
    {
        ...
        QLIST_FOREACH_ENTRY(memories, entry){
            printf("Checking if need to connect IRQ\n");
            g_assert(qobject_type(entry->value) == QTYPE_QDICT);
            printf("Checking if need to connect IRQ\n");
            QDict * mapping = qobject_to(QDict, entry->value);
            connect_irqs(mapping, peripherals);
        }
    }
}

static void configurable_machine_instance_init(Object *obj){

}

static char *configurable_get_config_filename(Object *obj, Error **errp)
{
    ConfigurableMachineState *cms = CONFIGURABLE_MACHINE(obj);
    return g_strdup(cms->config_filename);
}

static void configurable_set_config_filename(Object *obj, const char *value, Error **errp)
{
    ConfigurableMachineState *cms = CONFIGURABLE_MACHINE(obj);
    g_free(cms->config_filename);
...
static const TypeInfo configurable_machine_type = {
    .name       = TYPE_CONFIGURABLE_MACHINE,
    .parent     = TYPE_MACHINE,
    .class_size = sizeof(ConfigurableMachineClass),
